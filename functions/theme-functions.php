...
if (!function_exists('responsi_header')) {
    ...
    function responsi_header()
    {
        ...
        <div id="responsi-wrapper" class="responsi-wrapper clearfix<?php if ('true' === $responsi_layout_boxed) {
            echo ' site-width';
                                                                   }?>">
        ...
        if ('one-col' !== $layout) {
            ...
            if (is_array($sidebar_args)) {
                if (responsi_active_sidebar(esc_attr($sidebar_args['sidebar_id']))) {
                    ...
                    <div id="<?php echo esc_attr($sidebar_args['sidebar_id']);?>" class="box<?php echo esc_attr($sidebar_box);?><?php if (isset($sidebar_args['class']) && !empty($sidebar_args['class'])) {
                        echo ' '.esc_attr($sidebar_args['class']);
                             } ?>">
        ...
        if (in_array($layout, $layouts)) {
            ...
            if (is_array($sidebar_alt_args)) {
                if (responsi_active_sidebar(esc_attr($sidebar_alt_args['sidebar_id']))) {
                    ?>
                    <?php do_action('responsi_sidebar_before'); ?>
                    <div id="<?php echo esc_attr($sidebar_alt_args['sidebar_id']);?>" class="box<?php echo esc_attr($sidebar_alt_box);?><?php if (isset($sidebar_alt_args['class']) && !empty($sidebar_alt_args['class'])) {
                        echo ' '.esc_attr($sidebar_alt_args['class']);
                             } ?>">
                        <?php do_action('responsi_sidebar_wrap_before'); ?>
                        <div class="sidebar-ctn clearfix">
                        <div class="sidebar-in">
...
if (!function_exists('responsi_get_sidebar_main_before')) {
    ...
    {
        global $layout;
        if ('two-col-right' === $layout || 'three-col-middle' === $layout || 'three-col-right' === $layout) {
            get_sidebar();
        }
        if ('three-col-right' === $layout) {
            get_sidebar('alt');
...
if (!function_exists('responsi_get_sidebar_main_after')) {
    ...
    {
        global $layout;
        if ('two-col-left' === $layout || 'three-col-left' === $layout) {
            get_sidebar();
        }
        if ('three-col-left' === $layout || 'three-col-middle' === $layout) {
            get_sidebar('alt');
        }
    }
}
...
if (!function_exists('responsi_single_post_meta_date')) {
    ...
    function responsi_single_post_meta_date()
    {
        ...
        $output = sprintf(
            '<span class="posted-on i_dates"><span class="screen-reader-text">%1$s </span>%2$s</span>',
            _x('Posted on', 'Used before publish date.', 'responsi'),
            $time_string
        );
...
if (!function_exists('responsi_single_post_meta_author')) {
    ...
    function responsi_single_post_meta_author()
    {
        ...
        $output = sprintf(
            '<span class="i_authors"><span class="author vcard"><span class="screen-reader-text">%1$s </span> <a class="url fn n" href="%2$s">%3$s</a></span></span>',
